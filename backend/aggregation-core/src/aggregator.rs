...
Bug #8 — Conversões de proto


Coloquei o mapa de Timeframe no crate common (fonte única, já que ambos dependem de contracts-rs) e as conversões de candle/delta/tick/snapshot no aggregation-core. Fiz tudo como funções livres de propósito: impl From<...> pra tipo estrangeiro esbarra na orphan rule, e delta/close precisam de symbol/seq que o From não conseguiria injetar.



Novo arquivo backend/common/src/proto.rs:
